                        ...
                        <div class="radio">
                            <label>
                                <input onClick="viewDetail(this.id)" type="radio" name="optionsRadios" id="task<?= $t->task_id ?>" value="<?= $t->task_id ?>" <?= $task_id == $t->task_id ? ' checked' : ''?>>
                                    <?= $t->task_name.'<br><small>Effective '.date('d M Y ', strtotime($t->effective_date)).'</small>'?>
                                    <a href="<?= base_url('Task/copy_temp/'.$t->task_id)?> "><span class="badge">Copy to Temp</span></a>
                            </label>
                        </div>
                    <?php endforeach; ?>
                </div>
