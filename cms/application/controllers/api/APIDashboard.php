...
class APIDashboard extends REST_Controller
{
	...
	function addSchedule_post()
	{
		...
		if ($periode !== date('Ym', strtotime($visit_date))) {
		...
		$xml = $this->myxml->Serialize_Array($data);
		$xml = str_replace("'", "''", $xml);
		$rs = $this->m_dashboard->validSchedule($xml);

		if ($rs->Id != 0) {
			$this->response(array('success' => TRUE, 'message' => $rs->ProcessMessage), 200);
		} else {
			$this->response(array('success' => FALSE, 'message' => $rs->ProcessMessage, 'schedule_number' => $data['schedule_number'], 'kode' => $data['kode'], 'visit_date' => $data['visit_date'], 'visit_hour' => $data['visit_hour']), 200);
		}
	}
