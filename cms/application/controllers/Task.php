...
class Task extends CI_Controller {
	...
	function submit(){
		...
		$next_month = date('Y-m-31', strtotime("+1 month"));

		if (strtotime($effective_date) <= strtotime(date('Y-m-d')) || strtotime($effective_date) > strtotime($next_month)){
			header('Location:'.$uri);
			
		}else {
			$rs = $this->m_list->processTaskList($name, $effective_date);
	...
	function copy_temp($id = null){
		$this->m_list->copyTaskTemp($id);
		header('Location:'.base_url('Task/add'));
	}
