...
class APIReason extends REST_Controller {
	...
	function add_post()
    {
		 if(!$this->post('name'))
        {
			$this->response(array('success' => FALSE, 'message' => 'Alasan Kunjungan masih kosong'), 200);
        }
	
		
		$object = array(
				'reason_name' => $this->post('name'),
	...
	function edit_post()
    {
		...
		$obj = $this->mycrud->updateData('reason_id', $this->post('id'), $object);
		if (!empty($obj)){
