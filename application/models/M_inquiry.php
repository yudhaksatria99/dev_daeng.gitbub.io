...
class M_inquiry extends CI_Model
{
	...
	{
		$query  = $this->db->query("EXEC dbo.spGetScheduleByNIK2 '$this->ho','$nik','$awal','$akhir'");
		return $query->result();
	}
	...
	function viewOtherapp($nik, $password)
	{
		$image_url = IMAGE_URL;
		$query  = $this->db->query("EXEC dbo.spGetOtherApp '$nik','$password','$image_url'");
		return $query->result();
	}
}
