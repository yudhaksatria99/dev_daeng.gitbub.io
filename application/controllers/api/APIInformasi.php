...
class APIInformasi extends REST_Controller {
	...
	function add_post()
    {
		 if(!$this->post('name'))
        {
			$this->response(array('success' => FALSE, 'message' => 'Sumber Informasi masih kosong'), 200);
        }
	
		
		$object = array(
				'informasi' => $this->post('name'),
	...
	function edit_post()
    {
		...
		$obj = $this->mycrud->updateData('informasi_id', $this->post('id'), $object);
		if (!empty($obj)){
