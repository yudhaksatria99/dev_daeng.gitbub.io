...
class APIDepartement extends REST_Controller {
	...
	function add_post()
    {
		 if(!$this->post('name'))
        {
			$this->response(array('success' => FALSE, 'message' => 'Nama Departement masih kosong'), 200);
        }
	
		
		$object = array(
				'dept_name' => $this->post('name'),
	...
	function edit_post()
    {
		...
		$obj = $this->mycrud->updateData('dept_id', $this->post('id'), $object);
		if (!empty($obj)){
